...
def densenet121_ibn_a(last_stride=1, pretrained=False, **kwargs):
    r"""Densenet-121-IBN-a model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNet_IBN(last_stride, num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    return model


def densenet169_ibn_a(last_stride=1, **kwargs):
    r"""Densenet-169-IBN-a model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DenseNet_IBN(last_stride, num_init_features=64, growth_rate=32, block_config=(6, 12, 32, 32),
                     **kwargs)
    return model
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, last_stride=2):
...
class DenseNet_IBN(nn.Module):
    ...
    """
    def __init__(self, last_stride, growth_rate=32, block_config=(6, 12, 24, 16),
            ...
            if i != len(block_config) - 1:
                if i == 2:
                    trans = _Transition(num_input_features=num_features,
                                        num_output_features=num_features // 2, last_stride=last_stride)
                else:
